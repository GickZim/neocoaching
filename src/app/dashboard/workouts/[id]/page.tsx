...
import {
  ArrowLeft, Download, FileText, Loader2, CheckCircle, Flame, Battery,
} from "lucide-react";
...
export default function ClientWorkoutDetailPage() {
  ...
  useEffect(() => {
    async function loadSession() {
      ...
      const { data } = await supabase.from("workout_sessions").select("*")
        .eq("user_id", userId).eq("day_id", activeDayId).eq("logged_date", today).single();
      if (data) {
        ...
        setCompleted(false); setNotes(""); setDifficulty(null); setEnergy(null);
      }
    }
    loadSession();
  ...
  async function saveSession() {
    ...
    const { error } = await supabase.from("workout_sessions").upsert({
      user_id: userId, plan_id: plan.id, day_id: activeDayId, logged_date: today,
      completed, notes, difficulty, energy,
    }, { onConflict: "user_id,day_id,logged_date" });
    setSavingSession(false);
    if (error) { showToast(error.message, "error"); return; }
    showToast(completed ? "Workout marked complete! 💪" : "Session saved.", "success");
  }

  if (loading) return <div className="flex justify-center py-32"><Loader2 className="animate-spin text-[#D4AF37]" size={28} /></div>;
  if (!plan) return <p className="text-white/40 text-center py-32">Workout plan not found.</p>;
  ...
  return (
    <div className="pb-12 max-w-2xl">
      ...
      <div className="bg-gradient-to-br from-[#1a1200] to-[#0a0a0a] border border-[#D4AF37]/20 rounded-2xl p-6 mb-6">
        ...
        <p className="text-white/35 text-sm mb-4">Coach Neo {plan.duration_weeks ? `· ${plan.duration_weeks} week program` : ""}</p>
        {plan.pdf_url && (
        ...
        {plan.workout_days.map(day => (
          <button key={day.id} onClick={() => setActiveDayId(day.id)}
            className={`shrink-0 px-4 py-2.5 rounded-xl text-sm font-semibold whitespace-nowrap transition-all ${
              activeDayId === day.id ? "bg-[#D4AF37] text-black" : "bg-white/4 border border-white/8 text-white/50 hover:text-white"
            }`}>
            {day.name}
          </button>
        ))}
      ...
      {/* Exercises */}
      {activeDay && (
        <motion.div key={activeDay.id} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }} className="space-y-3 mb-8">
          {activeDay.workout_exercises.length === 0 ? (
            <p className="text-white/30 text-sm text-center py-12">No exercises added for this day yet.</p>
          ) : activeDay.workout_exercises.map(ex => (
            userId && <ExerciseLogCard key={ex.id} exercise={ex} userId={userId} planId={plan.id} dayId={activeDay.id} logDate={today} />
          ))}
        </motion.div>
      )}
      ...
      {activeDay && (
        <div className="bg-[#0a0a0a] border border-white/6 rounded-2xl p-6 space-y-5">
          <h2 className="font-bold text-sm text-white/50 uppercase tracking-wider">Finish {activeDay.name}</h2>

          <button onClick={() => setCompleted(c => !c)}
            className={`w-full flex items-center justify-center gap-2 py-3.5 rounded-xl font-bold text-sm transition-all ${
              completed ? "bg-green-500/15 border border-green-500/30 text-green-400" : "bg-white/4 border border-white/10 text-white/60 hover:text-white"
            }`}>
            <CheckCircle size={16} /> {completed ? "Workout Completed" : "Mark Workout Complete"}
          </button>
          ...
          <div>
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-sm font-semibold">Perceived Difficulty</span>
              <span className="ml-auto text-sm font-black text-[#D4AF37]">{difficulty ?? "—"}</span>
            </div>
          ...
          <div>
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="ml-auto text-sm font-black text-emerald-400">{energy ?? "—"}</span>
            </div>
            <div className="flex gap-1.5">
              {Array.from({ length: 10 }, (_, i) => i + 1).map(n => (
          ...
          <div>
            <label className="text-xs text-white/40 mb-1.5 block">Workout Notes</label>
            <textarea value={notes} onChange={e => setNotes(e.target.value)} placeholder="How did it feel? Anything to flag for your coach?"
              className="field-premium h-20 resize-none text-sm" />
          </div>

          <button onClick={saveSession} disabled={savingSession} className="btn-gold w-full text-sm py-3">
