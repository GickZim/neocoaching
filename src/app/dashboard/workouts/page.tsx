  ...
  useEffect(() => {
    async function load() {
      ...
      const { data } = await supabase
        ...
        .select("assigned_at, workout_plans(*)")
        ...
        const mapped = data
          .map((row) => {
            const plan = Array.isArray(row.workout_plans)
              ? row.workout_plans[0]
              : row.workout_plans;
            return plan ? { ...plan, assigned_at: row.assigned_at } : null;
          })
      ...
      <div className="flex items-center justify-between mb-8 flex-wrap gap-4">
        <div>
          ...
          <p className="text-white/35 text-sm mt-1">
            Track every set, rep and rest day.
          </p>
        </div>
        <Link
          href="/dashboard/workouts/history"
          className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-white/4 border border-white/8 hover:border-white/15 text-sm font-semibold text-white/60 hover:text-white transition"
        >
      ...
      {plans.length === 0 ? (
        <div className="bg-[#0a0a0a] border border-white/5 rounded-2xl p-16 text-center">
          ...
          <p className="text-white/30 text-sm">
            Your coach will assign you a program soon. Check back here.
          </p>
        </div>
      ) : (
        <div className="grid sm:grid-cols-2 gap-4">
            ...
            >
              ...
              >
                <div className="flex items-start justify-between mb-4">
                  ...
                  <ArrowRight
                    size={16}
                    className="text-white/20 group-hover:text-[#D4AF37] group-hover:translate-x-1 transition-all"
                  />
                </div>
                <h2 className="font-bold text-lg mb-1">{plan.title}</h2>
                {plan.goal && (
                  <p className="text-[#D4AF37] text-xs font-semibold mb-3">
                    {plan.goal}
                  </p>
                )}
                {plan.description && (
                ...
                <div className="flex items-center gap-3 text-xs text-white/30">
                  {plan.duration_weeks && (
                    <span className="flex items-center gap-1">
                      <Calendar size={11} /> {plan.duration_weeks} weeks
                    </span>
                  )}
                  {plan.pdf_url && (
                    <span className="flex items-center gap-1">
