...
type LogRow = { exercise_id: string; logged_date: string; sets_data: { weight_kg: number | null; reps_completed: number | null }[] };
...
export default function WorkoutHistoryPage() {
  ...
  const [prs, setPrs] = useState<{ exercise_name: string; weight_kg: number; reps: number; achieved_date: string }[]>([]);
  ...
  useEffect(() => {
    async function load() {
      ...
      const [{ data: logData }, { data: prData }, { data: sessions }] = await Promise.all([
        supabase.from("workout_logs").select("exercise_id, logged_date, sets_data, workout_exercises(name)").eq("user_id", user.id).order("logged_date", { ascending: true }),
        supabase.from("exercise_prs").select("*").eq("user_id", user.id).order("achieved_date", { ascending: false }),
        supabase.from("workout_sessions").select("completed").eq("user_id", user.id),
      ]);
      ...
      if (logData) {
        ...
        logData.forEach((l) => {
          const ex = Array.isArray(l.workout_exercises) ? l.workout_exercises[0] : l.workout_exercises;
          if (ex?.name) uniqueExercises.set(l.exercise_id, ex.name);
        });
        const exList = Array.from(uniqueExercises.entries()).map(([id, name]) => ({ id, name }));
        setExercises(exList);
        if (exList.length) setSelectedExercise(exList[0].id);
      }
      if (prData) setPrs(prData);
      if (sessions?.length) {
        setCompliance(Math.round((sessions.filter(s => s.completed).length / sessions.length) * 100));
      }
      setLoading(false);
    }
    load();
  ...
  const chartData = exerciseLogs.map(l => {
    const bestSet = l.sets_data.reduce((best, s) => (s.weight_kg ?? 0) > (best.weight_kg ?? 0) ? s : best, l.sets_data[0] || { weight_kg: 0, reps_completed: 0 });
    return {
      date: new Date(l.logged_date).toLocaleDateString("en-US", { month: "short", day: "numeric" }),
      weight: bestSet?.weight_kg ?? 0,
      volume: l.sets_data.reduce((s, set) => s + ((set.weight_kg ?? 0) * (set.reps_completed ?? 0)), 0),
    };
  });

  const bestEver = exerciseLogs.reduce((best, l) => {
    const bestSet = l.sets_data.reduce((b, s) => (s.weight_kg ?? 0) > (b.weight_kg ?? 0) ? s : b, { weight_kg: 0, reps_completed: 0 });
  ...
  return (
    <div className="pb-12">
      <Link href="/dashboard/workouts" className="inline-flex items-center gap-1.5 text-white/40 hover:text-white text-sm mb-6 transition">
      ...
      <p className="text-white/35 text-sm mb-8">Track your progression over time.</p>
      ...
      {exercises.length > 0 ? (
        <div className="bg-[#0a0a0a] border border-white/6 rounded-2xl p-6 mb-8">
          ...
          {bestEver.weight_kg > 0 && (
            <p className="text-white/35 text-sm mb-4">Best set: <span className="text-[#D4AF37] font-bold">{bestEver.weight_kg}kg × {bestEver.reps_completed}</span></p>
          )}
          <ResponsiveContainer width="100%" height={240}>
            <LineChart data={chartData}>
              ...
              <Line type="monotone" dataKey="weight" stroke="#D4AF37" strokeWidth={2.5} dot={{ fill: "#D4AF37", r: 4 }} name="Top Set (kg)" />
            </LineChart>
          </ResponsiveContainer>
        </div>
      ) : (
        <div className="bg-[#0a0a0a] border border-white/5 rounded-2xl p-16 text-center mb-8">
          ...
          <p className="text-white/30 text-sm">Log your first workout to see progression charts here.</p>
        </div>
      )}
            ...
            {prs.map((pr, i) => (
              <motion.div key={i} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.05 }}
                ...
                <div className="flex items-center gap-3">
                  <span className="text-[#D4AF37] font-bold text-sm">{pr.weight_kg}kg × {pr.reps}</span>
                  <span className="text-white/25 text-xs">{new Date(pr.achieved_date).toLocaleDateString("en-US", { month: "short", day: "numeric" })}</span>
                </div>
              </motion.div>
            ))}
          </div>
